class Interface
  ...
  def railway_wagons_list
    ...
    Train.all.each_with_index { |s, i| puts "#{i + 1}. #{t.type.to_s} train, number - #{t.number}" }
    answer = gets.chomp.to_i
    Train.all[answer-1].railway_wagon_to_block { |rc, i| puts "Number: #{i + 1}. Type - #{rc.show_type.to_s}. Occuped place - #{rc.filled_volume}/#{rc.empty_volume}" }
  end
